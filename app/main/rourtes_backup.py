...
def controlpanel_home():
    ...
    return render_template('admin/admin_home.html', users=users, users_total=users_total, mentees=mentees,
                           mentees_total=mentees_total, mentors=mentors,
                           mentors_total=mentors_total, schools_total=schools_total,
                           unapproved_mentees_total=unapproved_mentees_total,
...
def search_results(search, user_type):
    ...
    return render_template('admin/search_results/{}.html'.format(select_string), title='Search Results',
                           results=results, searched=search_string)
...
def controlpanel_add_schools():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        new_school = School(school_status=True, school_name=form.name.data, school_email=form.email.data,
...
def personal_form(applicant_type, school_id):
    ...
    if is_unique(School, School.school_id, school_id, model2=None, field2=None, data2=None) is False:
        ...
        if request.method == 'POST' and form2.validate_on_submit():
            ...
            try:
                new_user = User(email=form2.email.data, user_type=applicant_type, school_id=school_id,
                                password=password, bio="", creation_date=creation_date, active=False)
            ...
            if applicant_type == 'mentee':
                ...
                if form.mentor_xperience.data == '>=2' and form.mentor_occupation.data != 'N':
                    ...
                    new_user.personal_info.append(
                        PersonalInfo(carer_email="", carer_name="", status=form.mentor_occupation.data,
            ...
            new_user.hobbies.append(
                Hobbies(football=form.football.data, drawing=form.drawing.data, user_id=new_user.user_id))
            ...
            if applicant_type == 'mentee':
                return redirect(
                    url_for('main.location_form', applicant_type=applicant_type, applicant_id=new_user.user_id))
            elif applicant_type == 'mentor':
        ...
        flash(
            'Sorry you have entered an invalid registration link, please contact a system admin. CHANGE THIS TO REDIRECT')
        return redirect(url_for('main.home'))
...
def pairing(applicant_type, applicant_id, location):
    if applicant_type == 'mentee':
        ...
        db.session.commit()
        return render_template('profiles/mentor_profile.html', title='Mentor Profile', mentor=pair_with_mentor,
                               user=pair_with_user)
        ...
        db.session.add(new_pair)
        db.session.commit()
        return render_template('profiles/mentee_profile.html', mentee=pair_with_mentee, user=pair_with_user,
...
def book_meeting(pair_id, mentee_id, mentee_user_id):
    ...
    if request.method == 'POST' and form.validate_on_submit:
        ...
        if form.address.data == mentee_form.avoid_area:
            flash(
                "Sorry bud, your mentee doesn't feel comfortable going there. In the interest of their well-being, please pick another area!")
